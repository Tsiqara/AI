...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        minDistanceGhostPosition = None
        for ghostDis in livingGhostPositionDistributions:
            mostLikelyPosition = ghostDis.argMax()
            distance = self.distancer.getDistance(mostLikelyPosition, pacmanPosition)
            if distance < minDistance:
                minDistance = distance
                minDistanceGhostPosition = mostLikelyPosition

        bestAction = None
        minDistance = float("inf")
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            successorDistance = self.distancer.getDistance(successorPosition, minDistanceGhostPosition)
